...
def check_files_in_s3(prefix):
    s3 = boto3.client('s3')
    bucket_name = 'spotify-data-store-bucket'
...
def check_all_files(**kwargs):
    ...
    user_streams = check_files_in_s3('data/streams/')
    songs = check_files_in_s3('data/songs.csv')
    users = check_files_in_s3('data/users.csv')
...
def move_files_to_archived(**kwargs):
    ...
    source_prefix = 'data/streams/'
    dest_prefix = 'data/streams-archieved/'
